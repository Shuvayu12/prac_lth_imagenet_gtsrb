...
def setup_model_dataset(args, if_train_set=False):
    if args.dataset == 'cifar10':
        ...
        train_set_loader, val_loader, test_loader = cifar10_dataloaders(
            batch_size=args.batch_size, data_dir=args.data, dataset=if_train_set)
    ...
    elif args.arch == 'res18_tiny':       
        print('build model: ResNet18_TinyImageNet')
        model = ResNet18_TinyImageNet(num_classes=classes)

    elif args.arch == 'res18_gtsrb':
...
def setup_model_dataset_PIE(args):
    ...
    if args.dataset == 'cifar10':
        ...
        normalization = NormalizeByChannelMeanStd(
            mean=[0.4802, 0.4481, 0.3975], std=[0.2302, 0.2265, 0.2262])
        train_loader = tiny_imagenet_dataloaders_val(
            batch_size=args.batch_size, data_dir=args.data, split_file=args.split_file)

    elif args.dataset == 'gtsrb':
        classes = 43
        normalization = NormalizeByChannelMeanStd(
            mean=[0.3403, 0.3121, 0.3214], std=[0.2724, 0.2608, 0.2669])
        train_loader = gtsrb_dataloaders_val(
            batch_size=args.batch_size, data_dir=args.data, split_file=args.split_file)
...
def sorted_examples(example_wise_prediction, data_prune, data_rate, state, threshold, train_number):
    ...
    if data_prune == 'constent':
        ...
        raise ValueError('error data_prune type')
...
def blance_dataset_sequence(class_wise_sequence, num_class):
    ...
    class_wise_number = np.zeros(num_class, dtype=np.int_)
    for i in range(num_class):

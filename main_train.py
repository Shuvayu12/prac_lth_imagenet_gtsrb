...
def load_ticket(model, args):
    ...
    # weight 
    if args.init_dir:
        initalization = torch.load(args.init_dir, map_location = torch.device('cuda:'+str(args.gpu)), weights_only=False)
    ...
    # mask 
    if args.mask_dir:
        current_mask_weight = torch.load(args.mask_dir, map_location = torch.device('cuda:'+str(args.gpu)), weights_only=False)

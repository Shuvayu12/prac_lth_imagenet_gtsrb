...
parser.add_argument('--decreasing_lr', default='25,50,75', help='decreasing strategy')
...
def main():
    ...
    if args.resume:
        ...
        checkpoint = torch.load(args.checkpoint, map_location = torch.device('cuda:'+str(args.gpu)), weights_only=False)
...
def prune_aware_example(full_model_path, prune_model_path, criterion, args):
    ...
    test_model.cuda()
    full_model_path_checkpoint = torch.load(full_model_path, map_location = torch.device('cuda:'+str(args.gpu)), weights_only=False)['state_dict']
    ...
    # pruning model 
    prune_model_path_checkpoint = torch.load(prune_model_path, map_location = torch.device('cuda:'+str(args.gpu)), weights_only=False)['state_dict']
